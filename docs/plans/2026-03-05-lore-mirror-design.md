...
- `scripts/test_api.py`: 自动化 API 测试（50 个用例，零外部依赖，支持 `--url` 远程测试）
...
MCP Server 让 AI 通过结构化工具直接访问邮件列表，是 Skills 之外的另一种（推荐的）AI 集成方式。
...
详见 [API.md 的 MCP Server 章节](../API.md#mcp-server)。
