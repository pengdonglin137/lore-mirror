...
Base URL: `http://localhost:8000` by default (production) or `http://localhost:3000` (dev, proxied to :8000).
Ports are configurable — see [start.sh usage](#) and `LORE_PORT` / `LORE_DEV_PORT` env vars.
...
- REST API 运行中（MCP server 通过 httpx 调用本地 API）
...
```json
{
  "mcpServers": {
    "lore-mirror": {
      ...
      "args": ["/path/to/lore-mirror/server/mcp_server.py"],
      "env": {
        "LORE_API_URL": "http://localhost:9000"
      }
    }
  }
}
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `LORE_API_URL` | `http://localhost:8000` | REST API 基础地址（支持任意主机:端口） |
