...
# 自定义端口
./start.sh --port 9000              # API 端口（也可用 LORE_PORT=9000）
./start.sh --dev-port 4000          # 前端 dev server 端口（也可用 LORE_DEV_PORT=4000）
./start.sh --build --port 9000      # 生产模式自定义端口
```
...
| `GET /api/series?id={message_id}` | patch series 元数据（版本、trailer）；`&download=1` 下载 mbox |
| `GET /api/stats` | 全局统计 |
...
**前置条件：** REST API 必须运行中（MCP server 通过 `LORE_API_URL` 访问，默认 `http://localhost:8000`）。
...
> **注意：** `args` 中必须使用 `mcp_server.py` 的绝对路径。如果 API 不在默认端口，加 `env` 字段：
> ```json
...
```bash
export LORE_API_URL=http://localhost:9000
```
